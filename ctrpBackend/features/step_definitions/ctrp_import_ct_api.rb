...
Then(/^I want to verify the Primary Purpose$/) do |table|
  table.hashes.each do |row|
    if row["Primary Purpose"].eql?('Basic Science')
      Ct_api_helper.verify_primary_purpose(row["Primary Purpose"].to_s, @nct_id, @data_hash_ctgov)
    elsif row["Primary Purpose"].eql?('Diagnostic')
      Ct_api_helper.verify_primary_purpose(row["Primary Purpose"].to_s, @nct_id, @data_hash_ctgov)
    elsif row["Primary Purpose"].eql?('Health Services Research')
      Ct_api_helper.verify_primary_purpose(row["Primary Purpose"].to_s, @nct_id, @data_hash_ctgov)
    elsif row["Primary Purpose"].eql?('Prevention')
      Ct_api_helper.verify_primary_purpose(row["Primary Purpose"].to_s, @nct_id, @data_hash_ctgov)
    elsif row["Primary Purpose"].eql?('Screening')
      Ct_api_helper.verify_primary_purpose(row["Primary Purpose"].to_s, @nct_id, @data_hash_ctgov)
    elsif row["Primary Purpose"].eql?('Supportive Care')
      Ct_api_helper.verify_primary_purpose(row["Primary Purpose"].to_s, @nct_id, @data_hash_ctgov)
    elsif row["Primary Purpose"].eql?('Treatment')
      Ct_api_helper.verify_primary_purpose(row["Primary Purpose"].to_s, @nct_id, @data_hash_ctgov)
    elsif row["Primary Purpose"].eql?('Device Feasibility')
      Ct_api_helper.verify_primary_purpose(row["Primary Purpose"].to_s, @nct_id, @data_hash_ctgov)
    elsif row["Primary Purpose"].eql?('Other')
      Ct_api_helper.verify_primary_purpose(row["Primary Purpose"].to_s, @nct_id, @data_hash_ctgov)
    else
      flunk 'Please provide correct CTRP Primary Purpose. Provided CTRP Primary Purpose name <<' + row["Primary Purpose"].to_s + '>> does not exist'
    end
  end
end

And(/^Description should be populated$/) do
  #need to add description tag is populated method once other trial nct id discovered
end
